...
class TXI { // input d'une transaction
	public:
		...
		unsigned int nUtxo; // index of output ( la reference d'une transaction precedente)
		unsigned char signature[64]; // signature de la transaction avec la cle privee
};
...
class UTXO { // une sortie non d�pens�e
	public:
		...
		float montant;
		unsigned char dest[KEY_SIZE];  //	compte destinataire (cle publique)
